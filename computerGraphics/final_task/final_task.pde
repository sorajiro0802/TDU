...
void setup() {
  ...
  Light=new Point(3,0,-2);Light.Normalize(); // 光線ベクトル
}
...
void drawConerend() { 
  ...
  dt=2*PI/float(n);
   for(int i=0;i<n;i++){
     dti=dt*float(i); dtip1=dt*float(i+1);
     P1.SetValue(cos(dti),  sin(dti),  0.0);
     P2.SetValue(cos(dtip1),sin(dtip1),0.0);
     P3.SetValue(0.0,       0.0,       1.0);
     P4.SetValue(0.0,       0.0,       0.0); 
     rend_triangle(P1,P2,P3);
...
void drawCylinderrend() { 
 ...
 /* 以下、「表からみて反時計回り」に指定　*/
     ...
     rend_triangle(  P1, P6, P2  );     rend_triangle( P4, P5, P3 ); 
 }  
}
//void drawCTConerend(int n, int r, int R){
//  Point P1,P2,P3,P4,P5,P6;
//  float dti,dtip1,dt;
//  noStroke(); 
//  P1=new Point();P2=new Point();P3=new Point();P4=new Point();
...
//     dti=dt*float(i); dtip1=dt*float(i+1);
//     P1.SetValue( 0,            0,            0);
//     P2.SetValue( R*cos(dti),   R*sin(dti),   0);
//     P3.SetValue( R*cos(dtip1), R*sin(dtip1), 0);
//     P4.SetValue( 0,            0,            1);
//     P5.SetValue( r*cos(dti),   r*sin(dti),   1);
//     P6.SetValue( r*cos(dtip1), r*sin(dtip1), 1);
//  }
//  rend_triangle( P1, P2, P3); rend_triangle(P2, P3, P5);
//  rend_triangle( P4, P5, P6); rend_triangle(P5, P6, P3);
//}
void drawCTConerend(int n, int r, int R) { 
  Point P1,P2,P3,P4,P5,P6;
  float dti,dtip1,dt;
  ...
  dt=2*PI/float(n);
   for(int i=0;i<n;i++){
     dti=dt*float(i); dtip1=dt*float(i+1);
     P1.SetValue( R*cos(dti),   R*sin(dti),   0);
     P2.SetValue( R*cos(dtip1), R*sin(dtip1), 0);
     P3.SetValue( r*cos(dtip1), r*sin(dtip1), 1);
     P4.SetValue( r*cos(dti),   r*sin(dti),   1);
     P5.SetValue( 0,           0,         1);
     P6.SetValue( 0,           0,         0);
     rend_triangle(  P1, P2, P3  );     rend_triangle( P1, P3, P4 );  
     rend_triangle(  P2, P1, P6  );     rend_triangle( P4, P3, P5 ); 
 }  
}

void drawLatherend(int n){
  float[] px = new float[]{0,  0.5,  1.5,  2,  4.5,  4,  1.5,  1,    3,  0};
  float[] py = new float[]{-2,  0.5,  0.2,  1,  2.5,  3,  2.5,  3.8,  5,  4.5};
  ...
  for(int i = 0; i < n; i++){
    ...
    for(int j = 0; j < px.length-1; j++){
      P1.SetValue(px[j]*cos(dti),     px[j]*sin(dti),    py[j]);
      P2.SetValue(px[j]*cos(dtip1),   px[j]*sin(dtip1),  py[j]);
      P3.SetValue(px[j+1]*cos(dti),   px[j+1]*sin(dti),    py[j+1]);
      P4.SetValue(px[j+1]*cos(dtip1), px[j+1]*sin(dtip1),  py[j+1]);

      rend_triangle(  P1, P2, P3  );     rend_triangle( P3, P2, P4 );  
    }
  }
}
...
void keyPressed() {
  ...
  else if( key== 's' ) { saveFrame(saveName);println(saveName);} // Save File Name
}
